...
def _parse_triage_response(ticket: SupportTicket, ctx: AccountContext, response: str) -> TriageResult:
    ...
    # If parsing found nothing structured, store the full response
...
def triage_all(contexts: list[AccountContext]) -> list[TriageResult]:
    ...
    for ctx in contexts:
        ...
        for ticket in ctx.tickets:
            logger.info(f"Triaging {ticket.ticket_id} for {ctx.account_id}...")
            result = triage_ticket(ticket, ctx)
            results.append(result)

    # Check for gap accounts: tickets reference accounts not in our context map
    # (shouldn't happen with this dataset but guard anyway)
    serialised = [
        {
